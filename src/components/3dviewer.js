...
function Gems(props) {
  ...
  const Switcher = ({values,label,active,handler})=>{
    ...
    return (
      <div className={`flex justify-center items-center flex-row w-full`} role="group">
        <div className={`pr-2 w-14 text-right`}>{label}</div>
        {values.map((val)=>{
          return (
            <button type="button" className={`${baseclass} border ${val===active?activeclass:`bg-transparent`}`} onClick={(e)=>handler(e.target.value)} key={val} value={val}>
              {val}
            </button>
          )
        })}
  ...
  export default function Viewer3d() {
    ...
    function Loader() {
      ...
      <div className={`flex flex-col justify-center items-center h-[60vh] md:h-[85vh]`}>
        <div className={`z-40 bg-white grid gap-2 grid-rows-2 grid-cols-1 w-full pb-4`}>
