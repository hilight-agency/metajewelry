...
const ContactsPage = () => {
  ...
  return (
    <>
      ...
      />
      <div
        className={`bg-black text-white px-5 w-full h-screen flex justify-center items-center gap-2`}
      >
        <div
          className={`grid lg:px-5 grid-cols-1 gap-5 md:gap-0 md:container md:mx-auto md:grid-cols-2`}
        >
          <div className={`uppercase`}>
            ...
            <hr
              className={`hidden lg:block my-5 w-48 border-none h-px text-gray-400 bg-gray-400`}
            />
            <p className={`uppercase font-thin text-xl lg:text-2xl`}>
              {t({ id: `contacts.subtext` })}
            </p>
